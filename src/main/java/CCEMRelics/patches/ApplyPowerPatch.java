...
import CCEMRelics.relics.PaperClip;
    ...
    public static class ConstructorPatch {
        ...
        public static void Patch(ApplyPowerAction __instance, AbstractCreature target, AbstractCreature source,
            ...
            if (!(target == null) && !target.isPlayer) {
                if (AbstractDungeon.player.hasRelic(PaperClip.ID) && (powerToApply instanceof WeakPower) && __instance.amount >= 0) {
                    AbstractDungeon.actionManager.addToBottom(new DamageAction(target, new DamageInfo(target, 4), AbstractGameAction.AttackEffect.SLASH_DIAGONAL));
                }
            }
        }
    }
